/*
  ...
  1. New Tables
    - `scan_snapshots`
      - Stores periodic market scan results per instrument
      - Captures market state, setup score, and directional bias
      - Indexed for efficient latest-record lookups

    - `recommendations`
      - Stores trade setup recommendations generated by the engine
      - Contains entry, stop loss, target, and confidence
      - Full textual explanation for UI display
  ...
  3. Indexes
    - Composite index on instrument_key + timestamp for fast lookups
    - Index on score for sorting/filtering
    - Index on is_active for recommendation filtering
*/

-- Instruments table (idempotent seed for Supabase)
...
-- Indicator Snapshots (idempotent)
...
COMMENT ON TABLE scan_snapshots IS 'Periodic market scan results per instrument';
COMMENT ON TABLE recommendations IS 'Trade setup recommendations from the analysis engine';
